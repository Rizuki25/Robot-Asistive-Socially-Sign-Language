"""
Tahap 5: Prediksi Kata dari Video (Words Motion Pipeline)
=========================================================
Menjalankan model BiLSTM khusus kata terlatih pada satu video dataset dan
menampilkan atau menyimpan video dengan overlay kelas kata dan confidence.

Jalankan dari folder Model/:
    python -m src.words.predict_video --video_path dataset/words/raw_90/Makan/video_Makan_20260824_150230_078.avi
"""
...
def predict_sequence(
    ...
    """Extract motion features, pad, and classify one landmark sequence."""
    if landmarks.size == 0:
        raise ValueError("Tidak ada landmark yang berhasil diekstrak dari video")

    # Pastikan shape landmarks adalah (frames, 42, 3) untuk 2 tangan
    if landmarks.ndim != 3 or landmarks.shape[1:] != (42, 3):
        raise ValueError(
            f"Shape landmark {landmarks.shape} tidak cocok. Diharapkan (frames, 42, 3)."
        )
    ...
    if actual_features != expected_features:
        raise ValueError(
            ...
            f"model input_size={expected_features}."
        )

    inputs_tensor = torch.from_numpy(data).to(device)
    lengths_tensor = torch.from_numpy(lengths).to(device)

    model.eval()
    with torch.no_grad():
        logits = model(inputs_tensor, lengths_tensor)
...
def find_prediction_frame(
    ...
    # Fallback: tampilkan di akhir video jika jeda tidak ditemukan
...
def draw_landmarks(frame: np.ndarray, frame_landmarks: np.ndarray) -> None:
    ...
    # Warna: Slot 0 (Kiri) = Cyan, Slot 1 (Kanan) = Orange
...
def default_output_path(video_path: str, config: dict) -> str:
    """Build the default output path for an input video."""
    models_dir = config.get("paths", {}).get("models", "outputs/words_motion_90/models")
    base_dir = os.path.dirname(resolve_input_path(models_dir))
    pred_dir = os.path.join(base_dir, "predictions")
    filename = f"{os.path.splitext(os.path.basename(video_path))[0]}_prediction.mp4"
    return os.path.join(pred_dir, filename)


def resolve_save_path(user_path: str, video_path: str, config: dict) -> str:
    """Resolve a blank, directory, or explicit file path from the save prompt."""
    if not user_path.strip():
        return default_output_path(video_path, config)

    path = resolve_input_path(user_path.strip().strip('"'))
    extension = os.path.splitext(path)[1].lower()
    if os.path.isdir(path) or not extension:
        filename = os.path.basename(default_output_path(video_path, config))
...
def annotate_video(
    ...
    try:
        ...
        if display:
            cv2.namedWindow("Prediksi Kata - tekan Q atau Esc untuk keluar", cv2.WINDOW_NORMAL)
            cv2.resizeWindow("Prediksi Kata - tekan Q atau Esc untuk keluar", width, height)
        ...
        while True:
            ...
            if show_prediction:
                primary_text = f"Prediksi kata: {label}"
                secondary_text = f"Confidence: {confidence * 100:.2f}%"
                primary_color = (0, 255, 0)
            else:
                primary_text = "Gerakan sedang berlangsung..."
                secondary_text = "Prediksi tampil setelah gerakan selesai"
            ...
            if writer:
                writer.write(frame)
            if display:
                cv2.imshow("Prediksi Kata - tekan Q atau Esc untuk keluar", frame)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Prediksi kelas kata dari video dataset")
    parser.add_argument(
        "--video_path",
        default="dataset/words/raw_90/Makan/video_Makan_20260824_150230_078.avi",
        help="Path video input (default: dataset/words/raw_90/Makan/video_Makan_20260824_150230_078.avi)",
    )
    parser.add_argument(
        "--model_path",
        default=None,
        help="Path checkpoint model (default: otomatis dari config, misal outputs/words_motion_90/models/best_model.pth)",
    )
    parser.add_argument(
        "--config",
        default="configs/words_motion_90.yaml",
        help="Path konfigurasi (default: configs/words_motion_90.yaml)",
    )
    parser.add_argument(
        "--output_path",
        default=None,
        help="Path video hasil anotasi (opsional, format mp4)",
    )
    parser.add_argument(
        "--display_width",
        type=int,
    ...
    parser.add_argument(
        "--no_display",
        action="store_true",
        help="Jangan membuka window OpenCV (mode headless/server)",
    )
    ...
    try:
        ...
        if args.no_display and not args.output_path:
            ...
            if args.output_path:
                ...
                if answer in ("y", "ya"):
                    default_path = default_output_path(args.video_path, config)
                    user_path = input(
                        "Direktori/file tujuan "
                        f"(Enter untuk {default_path}): "
                    )
                    save_path = resolve_save_path(user_path, args.video_path, config)
                    annotate_video(
                        args.video_path,
                        save_path,
                        label,
